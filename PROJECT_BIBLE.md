...
**Step 1 — `/sample` blank page:** already fixed in a prior session (see 5c above) — re-verified working with a fresh `npm run dev` + real browser screenshot + click-to-flip before starting anything else.

**Step 3 — `/about` page:** dedicated new page (not added into `/projects`) — the bio is long and personal, and a new page is purely additive. Full verbatim bio, photo area is a clearly marked "Author photo coming soon" placeholder. Added to nav and as a new card on `/projects` (existing cards untouched). Found and fixed a real CSS Grid bug during verification (not just eyeballing the screenshot): `grid-cols-[1fr_1.6fr]` rendered as `1200px 115px` instead of a proportional split — classic "implied minimum size" issue from the `aspect-square` placeholder having no intrinsic content. Fixed with `min-w-0` on both grid children; confirmed via computed-style inspection that the columns then split ~357px/571px as intended. Committed `4574401`.

**Step 5 — `/archive` page:** lists articles (title/blurb/date/Share button), newest-first with a sort toggle. Data is a clearly-labeled hardcoded placeholder array (`src/components/archiveArticles.ts`) — one real example from the old Base44 site ("A Rich Man's War") plus three illustrative placeholders, file comments that it needs replacing once there's a real data source. `ShareButton.tsx` prefers the Web Share API, falls back to clipboard copy. Explicitly framed as a companion to `/writing`, not a replacement — `/writing` untouched. Verified both ShareButton code paths individually: headless Chromium exposed `navigator.share`, so a plain click only exercised that branch; re-tested with it stubbed to `undefined` to force the fallback and confirmed via `navigator.clipboard.readText()` that the right URL landed on the clipboard. Added "Archive" to nav. Committed `c688063`. **Open question for Kyler:** manual article entries or an RSS pull from Substack — not decided yet.

**Step 4 — `/reviews` page + Supabase:** see 5f above for the project itself. Table schema: id, name, role (nullable), review_text, approved (bool, default false), created_at. RLS enabled: anon can INSERT anything, but can only SELECT rows where `approved = true` — enforced at the database level, not just filtered in app code. Verified this actually holds by inserting an unapproved test row directly via the Supabase MCP tools and confirming the public REST endpoint didn't return it, then deleted the test row. Margaret's review seeded as pre-approved (real row in the table, not hardcoded in the page).

Found and fixed two real bugs via direct REST testing *before* writing any page code: (1) a plain `CREATE TABLE` doesn't grant the `anon` role table-level privileges the way Supabase's dashboard UI does — needed explicit `GRANT SELECT, INSERT ... TO anon` (plus sequence `USAGE`) on top of the RLS policies, which alone weren't sufficient; (2) requesting the row back after insert (`return=representation`, i.e. `.select()` in the JS client) failed with an RLS violation, because a freshly-submitted row defaults to `approved=false` and the read-back is itself subject to the SELECT policy — `ReviewForm.tsx`'s insert deliberately omits `.select()` and confirms success from the form's own state instead of a DB round-trip.
...
- [x] ~~Fix the blank `/sample` flipbook page~~ — resolved, re-verified multiple times since (see 5c)
