...
**Resolved same day — first attempt used the wrong asset, corrected same session.** Initially used `public/images/book-cover.jpg` (the angled 3D hardcover mockup rendered from the cover art, used by `BookCover.tsx` on `/book`) — Kyler flagged this as wrong immediately after. The actual source he meant was `assets/images/1062.jpg`: the original flat cover art (1536×1024, no 3D perspective/photo treatment) that the 3D mockup itself was rendered from. Copied it to `public/images/book-cover-flat.jpg` (kept as its own file — `book-cover.jpg` stays in place since `/book`'s 3D mockup still needs it) and pointed `CoverPage` in `FlipbookViewer.tsx` at that instead. Confirmed by looking at the actual image that title, subtitle, and author name are already legible on the cover art itself, so `CoverPage` just renders the image full-bleed via `next/image` with `fill` — no separate text overlay needed. Added a `.sample-page--front-cover` / `.sample-page__cover-image` CSS pair (`object-fit: cover`) instead of reusing the old `.sample-page--cover` text-page styling (kept for the "End of Sample" closer page, which still needs text layout). The flat art is still landscape (1536×1024) against a portrait flipbook page, so a plain centered crop clipped the edges of the subtitle line; tuned `object-position` to `44% center` — verified by iterating on real headless-browser screenshots (not guessed), including scrolling the flipbook fully into view before capturing (an early screenshot pass looked broken only because the viewport cut off the bottom of the page, not because of the actual crop) — so title, subtitle, and "Kyler Wakefield" all stay fully in frame at both desktop and mobile widths, with the page background color-matched to the image's own dark tone so there's no visible seam. Verified page-flip still advances correctly to page 1 afterward with zero console errors. `npm run build` passes.
