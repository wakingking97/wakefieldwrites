...
- [x] ~~Enable Vercel Analytics for this project~~ — confirmed by Kyler 2026-08-29, done via Vercel dashboard → Project → Analytics tab → Enable (that's where he got the `npm i @vercel/analytics` / `<Analytics />` steps, which were already live in the code from 5m). Site traffic collection is fully wired up end to end.
