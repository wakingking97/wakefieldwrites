...
- **Fixed: flipbook text getting cut off on several pages.** Root cause, found by inspecting computed styles up the DOM ancestor chain rather than guessing: `react-pageflip` sets `position: absolute` and an explicit pixel `height` via an **inline style** on its own page wrapper, and forces `display: block` — which silently overrides the `.sample-page { display: flex }` rule in `globals.css` (inline styles always beat stylesheet rules, regardless of specificity). That broke the flex-based scroll container completely: `.sample-page__inner` wasn't flex-constrained at all, so it just grew to fit its content (measured up to 1012px on a page with an ~460px visible box) and the excess visually spilled past the page's edge instead of scrolling. Confirmed by direct evidence — `clientHeight` matched `scrollHeight` exactly on every page regardless of content length, meaning nothing was actually constraining it. Fixed by switching `.sample-page__inner`/`.sample-page__number` to `position: absolute` + `inset`, which only needs a sized, non-static ancestor — both of which react-pageflip already provides inline. Re-verified via the same DOM inspection: `clientHeight` is now correctly bounded (461px) independent of content length, and content that exceeds it scrolls within its own box instead of spilling out.
- **Rebalanced the sample's pagination** so pages read more like an actual book: split the sections that were most overstuffed (The Method, The Structure, the Architecture 5-tool list, Before You Turn This Page), and — per Kyler's note that only the page where a section *begins* should carry its heading — stripped the eyebrow/heading from every continuation page (previously every one of the Introduction's multi-page run repeated "I Couldn't Unsee It" verbatim, which read mechanically rather than like a real book). Verified empirically by flipping through and measuring every page's `scrollHeight` vs. its `clientHeight`: 21 of 28 content pages now fit with zero scrolling, and the remaining 7 overflow only 9-112px (down from many pages overflowing 200-500px+ before) — confirmed via scroll-to-bottom screenshot that the worst case (the 5-tool list page) has every item still reachable, nothing lost.
- **Added gold flip-direction controls** per Kyler's ask for a visible affordance on the click zones: circular gold-bordered buttons with directional chevrons, wired to `react-pageflip`'s ref API (`pageFlip().flipNext()`/`.flipPrev()`), auto-disabling at the start/end. First attempt overlaid them on the book's left/right edges — worked on desktop but covered actual paragraph text on mobile, where the book fills nearly the full viewport width. Moved them into a control bar below the book instead (with a "Swipe, or tap the arrows to turn the page" hint), which never overlaps content at any width. Verified via headless browser on both viewports: buttons visibly enable/disable correctly, no text overlap, real touch-swipe still confirmed working. Committed `5ca155b`.
