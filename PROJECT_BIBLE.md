...
Separately, also found and fixed: **flipbook text getting cut off on several pages.** Root cause: `react-pageflip` sets `position: absolute` and an explicit pixel `height` via an **inline style** on its own page wrapper, and forces `display: block` — which silently overrides the `.sample-page { display: flex }` rule in `globals.css` (inline styles always beat stylesheet rules). That broke the flex-based scroll container completely: `.sample-page__inner` wasn't flex-constrained at all, so it just grew to fit its content and the excess visually spilled past the page's edge instead of scrolling. Fixed by switching `.sample-page__inner`/`.sample-page__number` to `position: absolute` + `inset`, which only needs a sized, non-static ancestor — both of which react-pageflip already provides inline. Committed `5ca155b`.

Also as part of that same pass: rebalanced the page manifest so pages read more like an actual book (split the most overstuffed sections; stripped the eyebrow/heading from every continuation page so only the page where a section *begins* carries it), added gold-bordered prev/next buttons with directional chevrons in a control bar below the book (an overlay-on-edges first attempt covered real text on mobile), added the site logo/favicon from `assets/images/1062.jpg` (`src/app/icon.png` + `public/images/logo.png`), and did a page-fullness pass (book height 500→475, paragraph type 0.82rem/1.7 → 0.86rem/1.72, `maxWidth` 500→400 so desktop wraps text like mobile does, plus two targeted splits of the worst outlier pages) after Kyler noted pages looked half-full — average fill 84% (up from 81%), worst-case overflow down from 251px to 113px. All verified via headless browser against the real production build: real synthetic touch-swipe, click-to-flip, prev/next button enable/disable states, zero console errors, screenshotted on both mobile and desktop. Committed `55dbea0`.
...
**Resolved:** this is the same image Kyler already dropped in as a real source file (`assets/images/1062.jpg`, not a screenshot capture) earlier this same day — already in use site-wide as the logo/favicon (`src/app/icon.png`, `public/images/logo.png`). Confirmed he has the actual file, not just something trapped in Base44's tools.
...
- [x] ~~Fix the blank `/sample` flipbook page~~ — resolved (see 5c), plus a follow-on text-cutoff fix, gold flip controls, and a page-fullness pass — verified via headless browser incl. real touch-swipe on mobile
- [x] ~~Confirm whether Kyler owns the book cover art source file from Base44~~ — yes, it's `assets/images/1062.jpg`, already in use as the site logo/favicon (see 5e)
...
- [ ] Redesign `/book` Store section: retailer buttons up top, "buy a signed copy directly" pitch below (pattern documented in 5e)
